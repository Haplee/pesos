...
export function SettingsPage() {
  ...
  const Toggle = ({ on }: { on: boolean }) => (
    <div className="w-11 h-6 rounded-full transition-colors relative" style={{ backgroundColor: on ? toggleOn : toggleOff }}>
      <div 
  ...
  return (
    <Layout>
      ...
      <div className="rounded-2xl p-4 mb-3" style={{ backgroundColor: bgCard, border: `1px solid ${border}` }}>
        ...
        <button onClick={() => setVibration(!vibration)} className="w-full flex items-center justify-between py-3 border-b" style={{ borderColor: border }}>
          <div>
      ...
      <div className="rounded-2xl p-4" style={{ backgroundColor: bgCard, border: `1px solid ${border}` }}>
        ...
        <div className="text-[0.85rem]" style={{ color: textMuted }}>GymLog v1.4</div>
